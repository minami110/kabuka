import { Detector } from '~/plugins/kabu_detector.js'
...
test('Detector::Detect', () => {
  ...
  // 日曜時点での予測
  let r = Detector.Detect(datas, dayIndex)
  expect(r.movingTypes).toEqual(['wave', 'poor', 'P3', 'P4'])

  // 日曜時点でデータ未入力
  datas = [null, null]
  r = Detector.Detect(datas, dayIndex)
  expect(r.movingTypes).toEqual(['wave', 'poor', 'P3', 'P4'])
...
test('Detector::detect_P4', () => {
  ...
  const dataA = [110, null, 95, 91, 86, 81, 75, 72, 115, 210]

  console.log('az')

  // 木曜AM時点で P3かP4
  timeIndex = 8
  r = Detector.Detect(dataA, timeIndex)
  ...
  console.log('nemu')
  const dataB = [98, null, 50, 46, 124, 131, 181, 191, 165]

  // 月曜AM
  timeIndex = 2
  r = Detector.Detect(dataB, timeIndex)
  // console.log(r)
  // expect(r.movingTypes).toEqual(["wave", "P4"])
  ...
  // 月曜AM type-c
  timeIndex = 2
  r = Detector.Detect(dataC, timeIndex)
  console.log('-- 月曜AM --')
  console.log(r)
